...
class VisionNode(Node):
    def __init__(self):
        ...
        self.sync = ApproximateTimeSynchronizer(
            ...
            queue_size=1, # controls the number of images which are stored which can avoid missed frames but adds delay
    ...
    def image_callback(self, img_msg, depth_msg):
        ...
        try:
            ...
            results = self.model(img, stream=True, half=True)
            # results = self.model(np.ascontiguousarray(img), stream=True)
            ...
            for r in results:
                ...
                for box in boxes:
                    ...
                    if conf > 0.3:
                        ...
                        # detected_objects.append(self.classNames[cls]) # just for debugging
            ...
            # detected_objects = list(set(detected_objects)) # just for debugging
            # self.get_logger().info(f"Detected objects: {detected_objects}") # just for debugging
            ...
            # self.image_pub.publish(ros_img) # publish the image as a topic (not really needed anymore)
